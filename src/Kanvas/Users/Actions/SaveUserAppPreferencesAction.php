...
class SaveUserAppPreferencesAction
{
    ...
    {
        // activeUserAppSettings is an array of keys that are allowed to be set,
        // this makes sure that no unwanted settings are stored.
        // This array of keys comes from the app setting in_app_user_settings_keys
        $activeUserAppSettings = $this->app->get('in_app_user_settings_keys');
        $savedAppPreferences = [];

        if (! is_array($activeUserAppSettings)) {
            return;
        }
