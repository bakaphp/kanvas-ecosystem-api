...
class EmitPolicyAction
{
    ...
    public function execute(): array
    {
        ...
        if ($quoteNumber === '') {
            throw new ValidationException('Order has no Universal Seguros quote number to emit');
        }

        $service = new UniversalSegurosService($this->order->app, $this->order->company);

        $service->emit($quoteNumber);
        $this->order->set(CustomFieldEnum::STATUS->value, InsuranceOrderStatusEnum::EMITTED->value);
