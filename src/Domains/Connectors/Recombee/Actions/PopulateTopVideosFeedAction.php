...
class PopulateTopVideosFeedAction
{
    ...
    public function execute(int $pageSize = 350): int
    {
        ...
        Message::fromApp($this->app)->whereHas('tags', function ($query) use ($topVideosSlug) {
            $query->where('slug', $topVideosSlug)
                ->where('messages.is_public', 1)
                ->where('messages.is_deleted', 0);
        })->get()->each(function ($message) use ($topVideosSlug) {
            $message->removeTag($topVideosSlug);
        });

        $topVideosMessages = Message::fromApp($this->app)
            ->where('is_public', 1)
            ->where('is_deleted', 0)
            ->where('created_at', '>=', now()->subDays($timePeriod))
            ->where('message_types_id', $this->messageType->getId())
            ->whereRaw("JSON_EXTRACT(message, '$.type') = 'video-format'")
        ...
        foreach ($topVideosMessages as $message) {
            try {
                ...
                // print_r('Added top tag to message ID: ' . $message->getId() . PHP_EOL);
            } catch (Exception $e) {
                // print_r('Failed to add top tag to message ID: ' . $message->getId() . PHP_EOL);
                continue;
            }
        }

        return count($topVideosMessages);
    }
}
