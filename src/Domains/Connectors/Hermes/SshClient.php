...
/**
 * SSH client for managing Hermes agent deployments on remote machines.
 *
...
class SshClient
{
    ...
    private function initFromCompany(): void
    {
        ...
        $this->hermesHome = $this->company->get(ConfigurationEnum::HERMES_HOME->value) ?? '~/.hermes';
        $this->cliPath = $this->company->get(ConfigurationEnum::CLI_PATH->value) ?? 'hermes';
        $this->configFilename = $this->company->get(ConfigurationEnum::CONFIG_FILENAME->value) ?? 'hermes.json';
    ...
    public static function fromMachine(AgentMachine $machine): self
    {
        ...
        $instance->hermesHome = '~/.hermes';
        $instance->cliPath = 'hermes';
        $instance->configFilename = 'hermes.json';
    ...
    /**
     * Run an Hermes CLI subcommand (e.g. "agents list --json").
     */
    ...
    {
        $configJson = $this->readFile($this->hermesHome . '/' . $this->configFilename);
    ...
    {
        return $this->exec('hermes gateway status --json --deep 2>/dev/null');
    }
    ...
    {
        return $this->exec('hermes status --json 2>/dev/null');
    }
    ...
    {
        return $this->exec('hermes node status --json 2>/dev/null');
    }
    ...
    {
        return $this->exec('hermes memory status 2>&1');
    }
    ...
    {
        return $this->exec('hermes config show --json 2>/dev/null');
    }
    ...
    {
        return $this->exec('hermes sessions list --json 2>/dev/null');
    }
     ...
     *
     * Commands run via `docker exec` against the agent's container because the hermes
    ...
    public function getAllTelemetryForContainer(string $containerName): array
    {
        ...
        //
        // `node /app/hermes.mjs` is used instead of the `hermes` alias because
    ...
    public function getAllTelemetry(): array
    {
        ...
        $script = implode('; ', [
            "echo '__SECTION__health'",
            "timeout 10 hermes health --json 2>&1",
            "echo '__SECTION__version'",
            "timeout 3 hermes --version 2>/dev/null || echo unknown",
            "echo '__SECTION__gateway'",
            "timeout 15 hermes gateway status --json --deep 2>/dev/null",
            "echo '__SECTION__memory'",
            "timeout 15 hermes memory status 2>&1",
        ]);
     ...
     * `hermes skills list`, which shows what's installed on the host, this reflects
     * what the specific agent has actually used.
     *
     * @param  string  $containerName  Docker container name running the agent (e.g. "hermes-whitco")
     * @param  string  $agentSlug      Hermes agent slug matching the agents/ sub-dir
    ...
    public function getAgentTools(string $containerName, string $agentSlug): ?string
    {
        ...
        // /home/node/.hermes/agents/<slug>/sessions/.
        $sessionsDir = '/home/node/.hermes/agents/' . $agentSlug . '/sessions';
        ...
        // `node /app/hermes.mjs` is used because `hermes` is not in PATH inside the container.
        $skillsRaw = $this->exec(
            'docker exec ' . escapeshellarg($containerName) . ' timeout 5 node /app/hermes.mjs skills list --json 2>/dev/null',
            10
        );
     ...
     *   /home/node/.hermes/agents/<agentSlug>/sessions/*.jsonl
     *
    ...
    private function extractTextContent(mixed $content): string
    {
        ...
        foreach ($content as $block) {
            ...
            if (($block['type'] ?? '') === 'text' && isset($block['text'])) {
                // Strip <final>...</final> wrapper hermes adds
