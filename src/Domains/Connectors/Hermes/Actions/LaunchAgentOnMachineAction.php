...
use Kanvas\Connectors\Hermes\Enums\DeploymentStatusEnum;
use Kanvas\Connectors\Hermes\Services\DockerComposeBuilder;
use Kanvas\Connectors\Hermes\Services\WorkspaceFileBuilder;
...
/**
 * Deploy an Hermes agent to a remote machine in full Docker isolation.
 *
 * Lifecycle:
 ...
 *  3. Write deployment files: Dockerfile, docker-compose.yml, hermes.json,
 ...
 * chowned to UID 1000 (node user inside the container) so the Hermes process
...
class LaunchAgentOnMachineAction
{
    ...
    public function execute(): AgentDeployment
    {
        ...
        try {
            ...
            $deployment->set(CustomFieldEnum::HERMES_GATEWAY_TOKEN->value, $gatewayToken);

            $this->agent->update(['deployment_status' => 'deployed']);
            $this->agent->set(CustomFieldEnum::HERMES_DEPLOYMENT_ID->value, $deployment->getId());
        } catch (Throwable $e) {
     ...
     * The user's home directory contains all Hermes config and workspace files.
     */
    private function provisionLinuxUser(SshClient $client, AgentDeployment $deployment): void
    {
        ...
        $client->exec('sudo mkdir -p ' . escapeshellarg($homeDir . '/.hermes/workspace'));
        $client->exec('sudo chown -R ' . escapeshellarg($user . ':' . $user) . ' ' . escapeshellarg($homeDir));
    }

    /**
     * Build the shared Hermes Docker image once per machine.
    ...
    private function ensureSharedImage(SshClient $client): void
    {
        ...
        if (! str_contains($result, 'EXIT_CODE:0')) {
            throw new ValidationException('Failed to build shared Hermes image: ' . $result);
        }
    }

    /**
     * Write all config files into the agent's ~/.hermes directory:
     *  - docker-compose.yml (gateway + socat proxy + CLI containers)
     *  - hermes.json (agent config, models, channels, gateway auth)
    ...
    private function writeDeploymentFiles(SshClient $client, AgentDeployment $deployment, string $gatewayToken): void
    {
        ...
        $client->writeFileAsUser(
            $hermesDir . '/hermes.json',
            DockerComposeBuilder::buildHermesConfig(
                $this->agent,
                $gatewayToken,
                $this->app,
        ...
        $agentDir = $hermesDir . '/agents/' . $this->agent->slug . '/agent';
        ...
        foreach ($files as $filename => $content) {
            $client->writeFileAsUser($hermesDir . '/workspace/' . $filename, $content, $systemUser);
        }
        ...
        $client->exec('sudo chown -R 1000:' . escapeshellarg($systemUser) . ' ' . escapeshellarg($hermesDir));
     ...
     * hermes.json and docker-compose.yml so the gateway config and container
    ...
    private function buildAndStart(SshClient $client, AgentDeployment $deployment, string $gatewayToken): void
    {
        ...
        for ($attempt = 1; $attempt <= $maxAttempts; $attempt++) {
            ...
            if ($this->hasPortBindError($result) && $attempt < $maxAttempts) {
                Log::warning('Hermes deployment hit Docker port bind conflict during launch', [
                ...
                $this->stopPartialDeployment($client, $deployment, $hermesDir);
    ...
    private function stopPartialDeployment(SshClient $client, AgentDeployment $deployment, string $hermesDir): void
    {
        $client->exec(
            'sudo -u ' . escapeshellarg($deployment->system_user)
    ...
    private function reassignDeploymentPorts(
        ...
        Log::info('Reassigned Hermes deployment ports after conflict detection', [
            'deployment_id' => $deployment->getId(),
            'machine_id' => $this->machine->getId(),
            'machine_name' => $this->machine->name,
            'attempt' => $attempt,
