<?php

declare(strict_types=1);

namespace Kanvas\Social\Enums;

enum AppEnum: STRING
{
    case SHAREABLE_LINK = 'APP_SHAREABLE_LINK';
    case SHAREABLE_PROFILE_LINK = 'APP_SHAREABLE_PROFILE_LINK';
    case SHAREABLE_LINK_WITH_USERNAME = 'APP_SHAREABLE_LINK_WITH_USERNAME';
    case MESSAGE_SEARCH_SUGGESTION_INDEX = 'APP_MESSAGE_SEARCH_SUGGESTION_INDEX';
    case ALLOW_APP_WIDE_USER_CHANNEL_ASSIGNMENT = 'APP_ALLOW_APP_WIDE_USER_CHANNEL_ASSIGNMENT';
}
