...
class PeoplesRepository
{
     ...
     * (every phone type, work phone included), so pass them pre-normalized
                    ...
                    ->where(function (Builder $q) use ($emails, $phones) {
                        if (! empty($emails)) {
                            $q->orWhere(function (Builder $e) use ($emails) {
                                $e->whereIn('contacts_types_id', Contact::EMAIL_TYPES)
                        ...
                        if (! empty($phones)) {
                            $q->orWhere(function (Builder $ph) use ($phones) {
                                $ph->whereIn('contacts_types_id', Contact::PHONE_TYPES)
