...
class EngagementLeadFilterServiceTest extends TestCase
{
    public function testUsesLatestStagePerEngagementEntity(): void
    {
        $records = collect([
            $this->engagement(1, 10, 'entity-a', 'credit-app', 'sent', '2026-01-01 10:00:00'),
            $this->engagement(2, 10, 'entity-a', 'credit-app', 'submitted', '2026-01-01 11:00:00'),
            $this->engagement(3, 20, 'entity-b', 'credit-app-2', 'opened', '2026-01-02 10:00:00'),
        ]);
        $service = $this->service($records);

        $incomplete = $service->resolve(app(Apps::class), auth()->user()->getCurrentCompany(), 'credit-app', 'incomplete');
        $submitted = $service->resolve(app(Apps::class), auth()->user()->getCurrentCompany(), 'credit-app', 'submitted');
    ...
    public function testMissingReturnsAnExclusionSelection(): void
    {
        $service = $this->service(collect([
            $this->engagement(1, 30, 'trade-a', 'add-trade', 'sent', '2026-01-01 10:00:00'),
        ]));

        $selection = $service->resolve(app(Apps::class), auth()->user()->getCurrentCompany(), 'trade-in', 'missing');
    ...
    private function service(Collection $records): EngagementLeadFilterService
    {
        return new class ($records) extends EngagementLeadFilterService {
