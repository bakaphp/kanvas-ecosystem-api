...
final class PullAllOrganizationsActionTest extends TestCase
{
    ...
    public function testAccumulatesRecordsAcrossAllPages(): void
    {
        ...
        Http::fake([
            // `?*` (not a bare `*`) so this doesn't also swallow the page-2 URL below — Http::fake
            // wildcards match path segments too, and `.../query*` alone matches `.../query/01gXX-2000`
            // just as well as `.../query?q=...`, which fed the paginator its own first page forever.
            ...
            self::SALESFORCE_INSTANCE_URL . '/services/data/v60.0/query/01gXX-2000' => Http::response([
                'totalSize' => 2,
                'done' => true,
                'records' => [['Id' => '001xx0000000002AAA', 'Name' => 'Globex Corp']],
            ], 200),
        ]);
        ...
        $this->assertSame('001xx0000000001AAA', $records[0]['Id']);
        $this->assertSame('001xx0000000002AAA', $records[1]['Id']);
        ...
        Http::assertSent(function ($request) {
            ...
            return str_starts_with($request->url(), self::SALESFORCE_INSTANCE_URL . '/services/data/v60.0/query')
                && str_contains((string) ($request['q'] ?? ''), 'FROM Account');
        });
    }
