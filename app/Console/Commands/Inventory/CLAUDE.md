...
| 4 | `kanvas-inventory:evaluate-product-discovery {app} {company} --file=golden.json [--k=10] [--min-recall=] [--show-misses]` | Scores discovery against a human-judged query set (recall@k, MRR). Exits non-zero under `--min-recall`, so it can gate a change. Without it, prompt tuning is guesswork. |
