...
class AgentMachineMutation
{
    ...
    public function updateContainers(mixed $root, array $request): bool
    {
        ...
        // Fan out across every registered runtime — a machine may host multiple runtime types
        // (OpenClaw + Hermes today, potentially more later). Each provider's job is idempotent
        // and short-circuits when its runtime isn't installed on the host.
        foreach (AgentRuntimeProviderFactory::runtimeProviders() as $provider) {
            $provider->dispatchUpdateMachineContainers($machine);
        }
