...
class DiscountManagementMutation
{
    ...
    public function update(mixed $root, array $request): Discount
    {
        ...
        $data = new DiscountData(
            ...
            start_date: isset($request['input']['start_date']) ? new DateTime((string) $request['input']['start_date']) : ($discount->start_date ? new DateTime($discount->start_date->format('Y-m-d H:i:s')) : null),
            end_date: isset($request['input']['end_date']) ? new DateTime((string) $request['input']['end_date']) : ($discount->end_date ? new DateTime($discount->end_date->format('Y-m-d H:i:s')) : null),
